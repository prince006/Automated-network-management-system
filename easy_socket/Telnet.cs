...
namespace easy_socket.Telnet
{
    ...
    public class Telnet
    {
        ...
        */
        ...
        int get_cursor_Y()
        {
            return get_relative_Y_pos(this.selection_start_to_num_line(this.iSelectionStart));
        }
        ...
        {
            return this.textbox.Lines.Length;
        }

        // remote terminal gives us position in terminal size position
        // so we have to translate y into textbox position
        // that means absoluteY=Max(Max(full_line_number-terminal_high,0)+relativeY,0)
        // and relativeY=Max(absoluteY-Max(full_line_number-terminal_high,0),0)
        int get_absolute_Y_pos(int relativeY)
        {
            return System.Math.Max(System.Math.Max(this.get_number_of_lines()-this.nb_char_high,0)+relativeY,0);
        }

        int get_relative_Y_pos(int absoluteY)
        {
            return System.Math.Max(absoluteY-System.Math.Max(this.get_number_of_lines()-this.nb_char_high,0),0);
        }
        ...
        void go_home()
        {
            this.set_cursor_pos(0, this.get_relative_Y_pos(0));// not for vt100
            this.textbox.ScrollToCaret();
            this.textbox.SelectionLength=1;
        }
        void go_end()
                            ...
                            switch (option_value)
                            {
                                case (byte)TELNET_OPTIONS.NAWS:
                                    ...
                                    this.al_reply_cmds.Add(new byte[]{(byte)TELNET_COMMANDS.IAC,(byte)TELNET_COMMANDS.SB,
                                                                         ...
                                                                         (byte)(this.nb_char_wide>>8),(byte)(this.nb_char_wide&0xff),
                                                                         (byte)(this.nb_char_high>>8),(byte)(this.nb_char_high&0xff),
